analysis_pos <- function(df,date1,date2,sentiment_threshold) {
...
analysis_p[[1]] <- df[df$pos >= sentiment_threshold | df$pos <= -sentiment_threshold,]
analysis_p[[2]] <- summary(df$pos)
analysis_p[[3]] <- var(df$pos)
analysis_p[[4]] <- skewness(df$pos)
...
plot(density(df$pos),main="Positive Sentiment Density Distribution")
analysis_p[[5]] <- recordPlot()

par(mfrow=c(1,1))
   plot(df$pos[2:length(df$wheat)],df_ret$wheat,main="Returns and Positive Sentiment Linear Model",xlab="Sentiment", ylab="Returns")
    abline(lm(df_ret$wheat ~ df$pos[2:length(df$wheat)]),col="red")
